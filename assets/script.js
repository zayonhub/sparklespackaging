...
(function siteChrome() {
  document.querySelectorAll(".nav-cta").forEach((btn) => {
    ...
    if (!btn.getAttribute("data-whatsapp"))
      btn.setAttribute(
        "data-whatsapp",
        "Hello Sparkles, I would like a packaging quote."
      );
  });
...
.product-capability-strip{display:grid;grid-template-columns:repeat(4,1fr);max-width:900px;width:100%;margin:0 auto 40px;background:#0A1258;color:#fff;border-radius:20px;overflow:hidden;box-shadow:0 14px 32px rgba(12,22,104,.14)}
...
.product-toolbar{display:flex;flex-direction:column;align-items:center;gap:16px;width:100%;max-width:900px;margin:0 auto 36px}
.product-search-wrap{position:relative;display:flex;align-items:center;width:100%;max-width:680px;margin:0 auto}
.product-search-wrap>svg{position:absolute;left:16px;color:#6B5D4F;pointer-events:none}
.product-search{width:100%;border:1.5px solid rgba(12,22,104,.14);border-radius:999px;background:#fff;padding:13px 90px 13px 46px;font:500 14px Inter,sans-serif;outline:none;box-shadow:0 4px 16px rgba(12,22,104,.06)}
...
.product-filters{display:flex;flex-wrap:wrap;justify-content:center;gap:8px;width:100%}
.product-filter{border:1px solid rgba(12,22,104,.14);background:#fff;color:#0C1668;border-radius:999px;padding:9px 14px;font:700 12px/1.2 Inter,sans-serif;cursor:pointer;transition:background .15s ease,color .15s ease,border-color .15s ease}
.product-filter:hover,.product-filter.is-active{background:#0C1668;color:#fff;border-color:#0C1668}
.modern-product-card{position:relative;overflow:hidden;background:#fff;border:1px solid rgba(12,22,104,.08);border-radius:24px;box-shadow:0 8px 28px rgba(12,22,104,.07);transition:transform .3s cubic-bezier(.22,1,.36,1),box-shadow .3s ease}
.modern-product-card:hover{transform:translateY(-6px);box-shadow:0 18px 40px rgba(12,22,104,.14)}
.modern-product-card::before{content:"";position:absolute;left:0;right:0;top:0;height:3px;background:linear-gradient(90deg,#0C1668,#1B33E0,#F5901F);z-index:3}
.modern-product-image{position:relative;aspect-ratio:3/2!important;overflow:hidden!important;background:#f4f7ff!important}
.modern-product-image>img{width:100%!important;height:100%!important;object-fit:cover!important;object-position:center!important;display:block!important;transition:transform .5s ease}
.modern-product-card:hover .modern-product-image>img{transform:scale(1.03)}
.modern-product-body{min-height:240px!important;padding:22px 22px 24px!important;display:flex;flex-direction:column}
...
.modern-product-body .portfolio-link{margin-top:auto;color:#0C1668!important;font-weight:800!important;background:none;border:none;padding:0;cursor:pointer;font:inherit;text-align:left}
