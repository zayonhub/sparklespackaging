...
(function siteUpgrade() {
  ...
  if (heading) {
    ...
    filters.innerHTML = categories.map((x, i) => `<button type="button" class="product-filter${i === 0 ? " is-active" : ""}" data-filter="${x.replace(/"/g, """)}">${x}</button>`).join("");
